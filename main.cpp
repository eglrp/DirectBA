...
void DirectAlignmentToFile(char *Path, DirectAlignPara &alignmentParas, vector<ImgData> &allImgs, vector<CameraData> &allCalibInfo, int pyrID)
{
	...
	{
		int cid = 0;
		int widthJ = allImgs[cid].imgPyr[pyrID].cols, heightJ = allImgs[cid].imgPyr[pyrID].rows, lengthJ = widthJ*heightJ;
		float *depthMap = new float[widthJ*heightJ];
		for (int ii = 0; ii < widthJ*heightJ; ii++)
			depthMap[ii] = 0.f;
		for (int ii = 0; ii < (int)invDAll[cid].size(); ii++)
		{
			double ij[3] = { indIAll[cid][ii], indJAll[cid][ii], 1 };
			depthMap[indIAll[cid][ii] + indJAll[cid][ii] * widthJ] = 1.0 / invDAll[cid][ii];
		}
		sprintf(Fname, "%s/D_%d_%d.dat", Path, cid, pyrID), WriteGridBinary(Fname, depthMap, widthJ, heightJ, 1);

		sprintf(Fname, "%s/3d_%d_%d.xyz", Path, cid, pyrID); FILE *fp = fopen(Fname, "w+");
		for (int ii = 0; ii < (int)invDAll[cid].size(); ii++)
		{
...
#include <math.h>   
double DirectAlignmentPyr(char *Path, DirectAlignPara &alignmentParas, vector<ImgData> &allImgs, vector<CameraData> &allCalibInfo, int fixPose, int fixDepth, int pyrID, int verbose = 0)
{
	char Fname[512];
	...
	/*{
		...
		FILE *fp = fopen("C:/temp/costY.txt", "w+");
		...
		//#pragma omp critical
		fprintf(fp, "%.3f %.3f %.3f %.16e\n", 0.005*x, 0.005*y, 0.005*z, sqrt(residuals / (0.001 + npts)));
		}
		}
		}
		fclose(fp);
	...
	//Data term
	ceres::LossFunction *ColorLoss = new ceres::TukeyLoss(10);
			...
			for (int cidI = 0; cidI < (int)allImgs.size(); cidI++)
			{
				...
				for (int ii = 0; ii < (int)invDAll[cidJ].size(); ii++)
				{
					...
					if (nchannels == 1)
					{
						...
						//resImg[indIAll[cidJ][ii] + indJAll[cidJ][ii] * widthJ] = (uchar)(int)(fcolor + 0.5);
					...
					else
					{
						...
						for (int kk = 0; kk < 3; kk++)
						{
							...
							//resImg[indIAll[cidJ][ii] + indJAll[cidJ][ii] * widthJ + kk*lengthJ] = (uchar)(int)(fcolor + 0.5);
							residuals += dif*dif;
						}
						npts += 3;
					}
				}
				sprintf(Fname, "%s/%d_%d_%d_%d.png", Path, cidJ, cidI, pyrID, iter), WriteGridToImage(Fname, resImg, widthJ, heightJ, nchannels);
	...
	if (fixDepth == 0)
		options.linear_solver_type = ceres::SPARSE_NORMAL_CHOLESKY; //depth or depth+pose
	else
		options.linear_solver_type = ceres::DENSE_QR; //pose
...
int DirectAlignment(char *Path, vector<ImgData> &allImgs, vector<CameraData> &allCalibInfo)
{
	...
	double colorSigma = 5.0, depthSigma = 0.02; //expected std of variables (grayscale, system unit);
	...
	for (int pyrID = 2; pyrID >= 2; pyrID--)
	{
		...
		for (int innerID = 0; innerID < innerIter; innerID++) //Pose is mostly well estimated even with nosiy depth. Depth is improved with good pose. -->Alternating optim, which is extremely helpful in practice
		{
			...
			innerCost1 = DirectAlignmentPyr(Path, alignmentParas, allImgs, allCalibInfo, 1, 0, pyrID);

			printf("\n@(Pose) Iter %d: ", innerID);
			innerCost2 = DirectAlignmentPyr(Path, alignmentParas, allImgs, allCalibInfo, 0, 1, pyrID);

			percentage = abs(innerCost1 + innerCost2 - pinnerCost1 - pinnerCost2) / (pinnerCost1 + pinnerCost2);
			printf("Change: %.3e\n", percentage);
			if (percentage < 1.0e-5)
				break;
		...
		//if (pyrID == 0)
			DirectAlignmentPyr(Path, alignmentParas, allImgs, allCalibInfo, 0, 0, pyrID, 0);
...
int main(int argc, char** argv)
{
	...
	if (mode == 2)
	{
		char Path[] = "D:/Source/Repos/Users/sudipsin/recon3D/data/Y";
		vector<ImgData> allImgs;
		vector<CameraData> allCalibInfo;
