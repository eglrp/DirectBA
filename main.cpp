...
int DirectAlignment(char *Path, vector<ImgData> &allImgs, vector<CameraData> &allCalibInfo, int smallAngle = 0)
{
	...
	double colorSigma = 5.0, depthSigma = 0.01; //expected std of variables (grayscale, system unit);
	...
	for (int pyrID = nscales; pyrID >= 2; pyrID--)
	{
		printf("\n\n@ level: %d\n", pyrID);
		...
		if (pyrID == 0)*/
			DirectAlignmentPyr(Path, alignmentParas, allImgs, allCalibInfo, 0, 0, pyrID, smallAngle, 0);
